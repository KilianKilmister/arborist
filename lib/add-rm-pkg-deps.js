...
const removeFromOthers = (name, type, pkg) => {
  ...
  // REVIEW: peerOverride: 'peerOptional' doesn't exist anymore
  switch (type) {
    case 'prod':
    ...
    // case 'peerOptional':
      ...
      // NOTE: commented out for now, not sure what the effects of leaving/removing this are
...
const addSingle = ({pkg, spec, saveBundle, saveType, path}) => {
  ...
  // REVIEW: peerOverride: block has no function without `peerOptional`
...
const getSaveType = (pkg, spec) => {
  ...
  if (devDeps && devDeps[name] !== undefined)
    ...
    return 'optional'
  // REVIEW: peerOverride: `peerOptional`-hook isn't needed now. Is a different save-type needed for peerOverrides?
