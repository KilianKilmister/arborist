...
// REVIEW: peerOverride: new symbol
const _peerDependenciesMeta = Symbol('_peerDependenciesMeta')
const PeerDepsMeta = require('./peerOverride.js')
...
class Node {
  constructor (options) {
    ...
    // NOTE: needs to be before child res, otherwise children would have a
    // parent but no parent peerDepsMeta
    this[_peerDependenciesMeta] = new PeerDepsMeta(this)
  ...
  [_loadDeps] () {
    ...
    /** @type {{ [peer: string]: string }} */
    const pd = this.package.peerDependencies
    // REVIEW: peerOverride: implementation
    if (pd && typeof pd === 'object' && !this.legacyPeerDeps) {
      // NOTE: **the relevant overrides are specified on the parent**
      // there is no possible reason for defining an override for a peer that you requested
      /** @type {Map<string, import('./peerDepOverrides').PeerOverride>} */
      const pm = this.parent && this.parent.peerDependenciesMeta.get(this.name)

      const peerDependencies = {}
      const peerOverride = {}

      for (let [name, spec] of Object.entries(pd)) {
        // if this package is mentioned in the **parents** `peerDependenciesMeta`
        // and the current entry is referenced, do the swap
        if (pm && pm.has(name)) {
          // extract the entry
          const override = pm.get(name)
          // record the originally requested spec
          override.origSpec = spec
          ;({ overridePackage: name, overrideSpec: spec } = override)

          // and add to override hash
          peerOverride[name] = spec
        } else
          //if no override is found, proceed as usual
          peerDependencies[name] = spec
      }
      // then continue as before
      this[_loadDepType](peerDependencies, 'peer')
      // changed from (hash: peerOptional, type: 'peerOptional')
      this[_loadDepType](peerOverride, 'peerOverride')
      // end of implementation, continue as usual
    }
  ...
  satisfies (requested) {
    ...
    return this.name === name && this.satisfies(new Edge({
      // NOTE: KK: assuming this is a dummy edge that is discarded afterwards
  ...
  // REVIEW: pdOverride: added getter
  /** @type {InstanceType<typeof PeerDepsMeta>} */
